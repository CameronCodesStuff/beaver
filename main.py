...
def install_model(large=False):
    ...
    def worker():
        ok = installer.install_model(progress, large=large)
        try:
            eel.installDone(ok)
        except Exception:
            pass
...
def start_listening():
    ...
    if speech is None:
        try:
            speech = SpeechEngine(installer.model_root(), on_partial=_on_partial)
...
@eel.expose
def toggle_mini(enabled):
    try:
        from core import window_ctl
        ok = window_ctl.set_mini(bool(enabled))
        return {"ok": ok}
    except Exception as e:
...
def _on_partial(text):
    # If the user starts speaking while Beaver is talking, interrupt it.
    if text and config.INTERRUPTIBLE and speaker.is_speaking():
        speaker.stop()
        try:
            eel.setStatus("LISTENING")
        except Exception:
            pass
    try:
        eel.setPartial(text)
